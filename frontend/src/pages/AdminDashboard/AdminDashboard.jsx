...
const AdminDashboard = () => {
  ...
  const handleStatusUpdate = async (id, status) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5050/api/payments/${id}`, 
        { status, remarks },
        { headers: { Authorization: 'Bearer mock-jwt-admin-token' } }
      );
      fetchPayments(page);
    ...
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 opacity-0 animate-fade-in relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <header className="mb-8 text-center sm:text-left flex flex-col sm:flex-row justify-between items-center glass p-5 rounded-2xl animate-slide-up">
          <div>
            <h1 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-indigo-800 tracking-tight">Financial Review Board</h1>
            <p className="text-slate-500 mt-1 text-xs font-medium tracking-wide">Evaluate student manual payments.</p>
          </div>
          <div className="mt-3 sm:mt-0 flex items-center space-x-2 bg-white/50 px-3 py-1.5 rounded-lg border border-slate-200">
              ...
              <tbody className="divide-y divide-slate-100 bg-white/40">
                {loading ? (
                  <tr>
                    <td colSpan="5" className="py-16 text-center">
                      <div className="inline-flex flex-col items-center">
                        <svg className="animate-spin h-6 w-6 text-indigo-500 mb-3" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                  ...
                  <tr>
                    <td colSpan="5" className="py-16 text-center text-slate-500 font-medium text-sm">
                      <div className="flex flex-col items-center">
                        <svg className="w-10 h-10 text-slate-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2v1m-6 5h6m-6 4h6m-6 4h6"/></svg>
                  ...
                  <tr key={payment._id} className="hover:bg-indigo-50/40 transition-colors duration-200 group">
                    <td className="py-4 px-5 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-500 font-medium mt-0.5">{new Date(payment.createdAt).toLocaleDateString()}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-5 whitespace-nowrap">
                      <div className="text-sm font-extrabold text-slate-900 tracking-tight">${payment.amount.toFixed(2)}</div>
                      <div className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider mt-0.5 bg-white inline-block px-1 rounded border border-slate-100">{payment.paymentFor}</div>
                    </td>
                    <td className="py-4 px-5 whitespace-nowrap text-center">
                      <div className="inline-block relative group/img cursor-pointer" onClick={() => setSelectedImage(`http://localhost:5050/${payment.slipImage}`)}>
                        <div className="w-16 h-10 rounded-md overflow-hidden border border-slate-200 bg-slate-100 group-hover/img:border-indigo-400 transition-colors shadow-sm relative">
                          <img src={`http://localhost:5050/${payment.slipImage}`} alt="Slip" className="w-full h-full object-cover" onError={(e) => { e.target.src = 'https://via.placeholder.com/150?text=Err'; }} />
                    ...
                    <td className="py-4 px-5 whitespace-nowrap text-center">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest shadow-sm  
                      ...
                      {payment.status === 'pending' ? (
                        <div className="flex flex-col items-end space-y-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                          ...
                          <div className="flex space-x-1.5 w-full">
                            <button onClick={() => handleStatusUpdate(payment._id, 'approved')} disabled={actionLoading === payment._id} className="flex-1 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white py-1 rounded text-[10px] font-bold shadow-sm transition-transform hover:-translate-y-0.5 disabled:opacity-50 disabled:transform-none">
                              ✓ Apprv
                            </button>
                            <button onClick={() => handleStatusUpdate(payment._id, 'rejected')} disabled={actionLoading === payment._id} className="flex-1 bg-gradient-to-r from-rose-500 to-rose-600 hover:from-rose-600 hover:to-rose-700 text-white py-1 rounded text-[10px] font-bold shadow-sm transition-transform hover:-translate-y-0.5 disabled:opacity-50 disabled:transform-none">
                              ✕ Rej
                            </button>
                          </div>
                        </div>
            ...
            <div className="bg-slate-50/80 px-5 py-3 flex items-center justify-between border-t border-slate-200 backdrop-blur-sm">
              ...
              <nav className="inline-flex rounded-md shadow-sm">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i} onClick={() => setPage(i + 1)} className={`px-3 py-1.5 text-xs font-bold transition-all first:rounded-l-md last:rounded-r-md border-y border-r first:border-l ${page === i + 1 ? 'z-10 bg-indigo-600 border-indigo-600 text-white shadow-sm' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
                    {i + 1}
                  </button>
                ))}
              </nav>
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-sm animate-fade-in" onClick={() => setSelectedImage(null)}>
          <div className="relative max-w-3xl max-h-[85vh] w-full animate-scale-in">
            <button className="absolute -top-10 right-0 w-8 h-8 rounded-full bg-white/10 hover:bg-rose-500 text-white flex items-center justify-center transition-colors border border-white/20 shadow-md" onClick={() => setSelectedImage(null)}>
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
            <div className="bg-white p-1.5 rounded-xl shadow-2xl">
              <img src={selectedImage} alt="Zoomed Slip" className="w-full h-full max-h-[75vh] object-contain rounded-lg" onClick={e => e.stopPropagation()} />
            </div>
          </div>
        </div>
      )}
