...
const { calculateReadiness } = require('./classOfferingService');
...
async function buildConfirmedStudentsReport(query) {
  ...
  const rows = enrollments.map((enrollment) => ({
    studentName: enrollment.student ? enrollment.student.fullName : 'Unknown student',
    email: enrollment.student ? enrollment.student.email : 'Unknown email',
    phone: enrollment.student ? enrollment.student.phone : 'No phone number',
    kuppiSession: enrollment.classOffering ? enrollment.classOffering.kuppiSession : 'Unknown session',
...
async function buildDispatchSummaryReport(query) {
  ...
  const failedRecipients = enrollments
    ...
    .slice(0, 10)
    .map((item) => ({
      studentName: item.student ? item.student.fullName : 'Unknown student',
      email: item.student ? item.student.email : 'Unknown email',
      kuppiSession: item.classOffering ? item.classOffering.kuppiSession : 'Unknown session',
    }));
  ...
  const recentDispatches = dispatchLogs.slice(0, 12).map((log) => ({
    ...
    status: log.status,
    subject: log.subject,
    studentName: log.student ? log.student.fullName : 'Unknown student',
    kuppiSession: log.classOffering ? log.classOffering.kuppiSession : 'Unknown session',
...
async function getReportPdfPayload(query) {
  const report = await buildReport(query);
